...
/* The class constructor is called by MainWindow and runs in the primary program
 * thread, this thread will go on to handle the GUI (mouse clicks, etc). The
 * OpenVRRenderWindowInteractor cannot be start()ed in the constructor, as it
 * will take control of the main thread to handle the VR interaction (headset
...
/* Standard destructor - this is important here as the class will be destroyed
 * when the user stops the VR thread, and recreated when the user starts it
 * again. If class variables are not deallocated properly then there will be a
 * memory leak, where the program's total memory usage will increase for each
 * start/stop thread cycle.
 */
...
void VRRenderThread::run() {
  ...
  while (!interactor->GetDone() && !this->endRender) {
     ...
     * This looks overcomplicated (and it is, C++ loves to make things
     * unecessarily complicated!) but is really just checking if more than 20ms
     * have elaspsed since the last animation step. The complications comes from
     * the fact that numbers representing time on computers don't usually have
     * standard second/millisecond units. Because everything is a class in C++,
     * the converion from computer units to seconds/milliseconds ends up looking
     * like what you see below.
     *
     * My choice of 20ms is arbitrary, if this value is too small the animation
     * calculations could begin to interfere with the interator processes and
     * make the simulation unresponsive. If it is too large the animations will
     * be jerky. Play with the value to see what works best.
     */
    if (std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now() - t_last)
            .count() > 20) {
...
void addCommand(std::shared_ptr<Commands::BaseCommand> command) { return; }
