...
class RenderThread : public QThread
{
  ...
  public:
    ...
    /**  Constructor
     */
    RenderThread(QObject *parent, vtkSmartPointer<vtkRenderer> renderer, vtkSmartPointer<vtkRenderWindow> window,
                 vtkSmartPointer<vtkRenderWindowInteractor> interactor, vtkSmartPointer<vtkCamera> camera);
  ...
  protected:
    ...
    // Custom callback for renderThread. Any modification of data inside this class should probably go through this
    // callback since vtkRenderWindowInteractor doesn't have DoOneEvent method, so handling the command in main loop
    // would be pretty much impossible. - Chanon Yothavut
    vtkSmartPointer<vtkTimerCallback> callback;

    // VTK rendering related things - I think this should be reassignable to vtkOpenVR alternative. - Chanon Yothavut
     ...
     */
    bool endRender; // Instead of setting this to true, might as well just stop the thread by calling
                    // renderThread->exit()

    // Need this to signal when the thread's renderWindow need to rerender. - Chanon Yothavut
    bool reRender;
    ...
    // vtkTimerCallback being a friend of RenderThread will make the code much more simpler, since there isn't a need
    // for getter/setter for protected member variable.
